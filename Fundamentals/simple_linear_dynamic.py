...
# Example: predict a simple single-variable linear relationship from x -> y
...
for epoch in range(epochs):
    # Clear and initialize gradients
    optimizer.zero_grad()
    y_hat = model(x_true)
    ...
    if (epoch + 1) % 50 == 0:
        ...
        plt.pause(0.8)  # Briefly pause to display the update
...
print(f"Actual Weight: {w}  Learned weights: {learned_weights[0][0].item():.2f}")
print(f"Actual Bias: {b}  Learned Bias: {learned_bias[0].item():.2f}")
plt.show()
